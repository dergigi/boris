...
it('unsubscribes immediately when the caller cancels', async () => {
  const source = new Subject()
  const controller = new AbortController()
  const result = queryEvents(pool(() => source), {}, { signal: controller.signal })
  source.next(sampleEvent)
  controller.abort()
  expect(source.observed).toBe(false)
  expect(await result).toEqual([sampleEvent])
})
it('does not subscribe for an already cancelled caller', async () => {
  const controller = new AbortController()
  controller.abort()
  const req = vi.fn()
  expect(await queryEvents(pool(req), {}, { signal: controller.signal })).toEqual([])
